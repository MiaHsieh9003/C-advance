...
void add_node(tNumStorHead *list){
    int tar_location =0; //target number location 
    int tar_loc_num=0;  //target number
    ...
    if(list->counts == 0){  //add the first number
        ...
        if(tar_location > list->counts || tar_location < 1){  //out of range in list
            ...
            list->counts ++;    //list number add 1
            next_ptr = list->head;
            for(int i=0 ; i<(tar_location) ; ++i){  //找到目標target number
                bef_ptr = next_ptr;                
            ...
            { 
                if(tar_location == 1){    //Before the location
                    if(!bef_ptr->bef){  //if insert before first number
                        ...
                        list->head = new_ptr;
                        return;                          
                    }else if(!next_ptr){   //所在next為null, insert before the last number
                        ...
                        bef_ptr = bef_ptr->bef; //將bef, next pointer分別指到欲插入的位置前後
                ...
                new_ptr->next = next_ptr;   //將new number加入bef, next之間
                bef_ptr->next = new_ptr;
                new_ptr->bef = bef_ptr;
